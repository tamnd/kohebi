...
2. Literal evaluation, since `Constant` holds a value rather than a token and the expression parser needs somewhere to put one. Done, with 148 hand-written cases and a sweep of every literal in the standard library.
3. Expressions: Pratt table, calls, subscripts, attributes, comprehensions, lambdas, conditional expressions, the walrus, f-strings and t-strings. Done, with 404 hand-written cases and a sweep of 552966 expressions out of the standard library that has no shape or position mismatches left. The only expressions the sweep still refuses are the one literal gap left, `\N{...}`.
...
Two known differences, both in position rather than text, both for item 9 to settle. CPython reports `unknown encoding:` and `codec can't decode byte` on line 0 with no source line, because they come out of a `LookupError` that has no position, and we report them on the line the cookie is on. The `Non-UTF-8` message, which is the one that actually happens, matches exactly, caret included.
...
Where the parser stands as of 29 August 2026: 1850 of the 1870 files in CPython 3.14.7's standard library parse to a tree whose dump is identical to CPython's, attributes and all. Nothing in the remainder is a wrong answer. All 20 are refusals kohebi reports as such, every one of them is `\N{...}`, and they do not appear in the token comparison because a tokenizer only has to hand back the text of a literal while a parser has to say what value it is. Lone surrogates were the larger half of that number until `Value::Str` grew a code point arm, and closing them moved 53 files.
...
| Tree agreement with `ast.parse` | 100% | 1850 of 1870 files, attributes included. No wrong answers, and the 20 left all want `\N{...}` |
