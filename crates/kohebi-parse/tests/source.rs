...
/// not text.
///
/// The traceback shows the line the error is on, and for these there is no line
/// until the file has been decoded, which is the thing that failed. The text
/// that comes back with the error is what makes it printable at all, and this
/// checks it lines up rather than pointing into the middle of a character.
#[test]
fn a_file_that_cannot_be_decoded_still_reports_itself() {
    let cases: &[(&[u8], &str, &str)] = &[
        (
            b"x = 1\ny = '\xe9'\n",
            "line 2",
            "Non-UTF-8 code starting with '\\xe9' on line 2",
        ),
        (
            b"# coding: cp1252\nx = '\x81'\n",
            "line 2",
            "'charmap' codec can't decode byte 0x81 in position 22",
        ),
        (
            b"\xef\xbb\xbf# coding: latin-1\nx = 1\n",
            "line 1",
            "encoding problem: iso-8859-1 with BOM",
        ),
    ];
    for (bytes, line, message) in cases {
        let error = decode(bytes).expect_err("these do not decode");
        let report = error.error.report(&error.text, "t.py");
        assert!(
            report.contains(line),
            "{bytes:?} should be reported on {line}:\n{report}"
        );
        assert!(
            report.contains(message),
            "{bytes:?} should say {message}:\n{report}"
        );
        assert!(
            report.contains('^'),
            "{bytes:?} should point at something:\n{report}"
        );
    }
}
